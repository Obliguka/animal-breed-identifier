...
if not _env_path.exists():
    _env_path = Path(__file__).parent.parent / "llm-observability" / ".env"
...
def trace_vision_call(prompt, image_base64, metadata=None, model="llava"):
    ...
    with _get_langfuse().start_as_current_observation(
        ...
        try:
            with _get_langfuse().start_as_current_observation(
                as_type="generation",
                name=f"llm_call_{model}_vision",
                model=model,
                input=prompt,
            ) as generation:
                ...
                if not result["success"]:
                    generation.update(output=None, level="ERROR", status_message=result.get("error_message", "Unknown error"))
